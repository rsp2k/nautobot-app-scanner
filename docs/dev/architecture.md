...
**Phase G' + J + L + L+1a postscript — closing the "dropdown lies" gap.**
...
TLS + SSH audits with native ordinal severity); **Phase L+1a added
`httpx`** as the first ProjectDiscovery-suite tool (modern HTTP
probe — JSONL output with ~30 fields including tech detection and
TLS handshake metadata). ToolChoices is now at **10 entries**. The
dispatch contract didn't change — only the registry filled out. The
...
adding tool #11 must keep all three registries in lockstep.
